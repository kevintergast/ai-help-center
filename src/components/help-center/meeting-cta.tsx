...
import { meetingHref, type MeetingConfig } from "@/lib/content/meeting";
...
export function MeetingCta({
  ...
  return (
    <a
      href={meetingHref(meeting, context)}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={`${meeting.label} — ${t("hc.meeting.opensExternally")}`}
      className={cn(
    ...
    >
      ...
      {meeting.description.length > 0 ? (
        <span className="text-[13px] leading-snug text-ink-muted">{meeting.description}</span>
      ) : null}
