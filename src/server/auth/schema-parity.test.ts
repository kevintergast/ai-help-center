...
describe("Schema-Parität: getAuthTables(tenantAuthOptions) ↔ migrations/", () => {
  ...
  it("signUp gegen die echten migrations/*.sql funktioniert und schreibt tenant_id", async () => {
    ...
    for (const f of ["0001_tenants.sql", "0021_tenant_suspend.sql", "0023_logo_dark.sql", "0025_header_name.sql", "0028_widget_on_site.sql", "0031_favicon.sql", "0002_auth.sql", "0004_two_factor_plugin_columns.sql"]) {
