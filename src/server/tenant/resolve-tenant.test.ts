...
describe("resolveWithSourceStrict (fail-closed — einzige Auflösungs-Variante)", () => {
  it("löst per Subdomain-Slug auf", async () => {
    expect((await resolveWithSourceStrict(source, "acme.hallofhelp.com"))?.slug).toBe("acme");
  });
  ...
  it("gibt bei unbekannter Instanz NULL zurück (kein Default-/Demo-Fallback)", async () => {
    expect(await resolveWithSourceStrict(source, "unknown.hallofhelp.com")).toBeNull();
    expect(await resolveWithSourceStrict(source, "hallofhelp.com")).toBeNull();
