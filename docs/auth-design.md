...
| D9 | **`cookieCache` global AUS** (`session.cookieCache.enabled:false`), nicht nur im Guard. **Nie** Parent-Domain-Cookie (`Domain=.hallofhelp.app`); host-scoped Cookies pro Origin, kein `crossSubDomainCookies`. | Verzögerte Widerrufe + Browser teilt A-Cookies an B (T-5, D9-alt). |
